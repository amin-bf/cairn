...
| ~~**A run completing mid-session shifts the queue**, and no local lockout can fix it (see Requirements).~~ — **fixed by [ADR-0015 §2](0015-the-sync-experience.md)**, and it was unfixable exactly as stated *until the trigger decision existed*: foreground-only sync triggers mean **sync never starts while the review screen is up**, which supplies a gate on *sync* rather than on review. A choice taken on packaging grounds turns out to buy the session's stability | — |
