...
//! and it is compared between two files of the same deck to make *"same revision, different bytes"*
//! reportable rather than silent (ADR-0008 §4). Both need a stable, collision-resistant digest over
//! the payload bytes; neither needs a keyed or password hash, which ADR-0020 forbids this crate
//! anyway.
...
/// The SHA-256 of `bytes`, as lowercase hex — the form a manifest carries and a comparison reads.
...
fn sha256(input: &[u8]) -> [u8; 32] {
    ...
    // Pad: the message, a 0x80 byte, zeroes to 56 mod 64, then the 64-bit big-endian bit length.
...
mod tests {
    ...
    fn matches_known_vectors() {
        // The two canonical FIPS 180-4 test vectors, plus the empty string.
        assert_eq!(
            sha256_hex(b""),
