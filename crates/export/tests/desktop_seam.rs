...
//!
//! There is no fake filesystem here for the same reason `leitner-store` has no fake store: the seam
...
//! environment can assert against — that is [#98](https://github.com/amin-bf/leitner/issues/98)'s.
...
fn put_reads_back_the_deduped_name_and_list_finds_it() {
    ...
    let first = platform::put("French A1.ldeck", b"deck-bytes").unwrap();
    assert_eq!(first.name, "French A1.ldeck");
    assert_eq!(platform::get("French A1.ldeck").unwrap(), b"deck-bytes");
    ...
    let second = platform::put("French A1.ldeck", b"other-bytes").unwrap();
    assert_eq!(second.name, "French A1 (1).ldeck");
    assert_eq!(
        platform::get("French A1 (1).ldeck").unwrap(),
        b"other-bytes"
    );

    // A file we do not recognise is invisible to the list; both `.ldeck` files are present, sorted.
    std::fs::write(dir.path().join("notes.txt"), b"not ours").unwrap();
    let listed = platform::list().unwrap();
    assert_eq!(listed, vec!["French A1 (1).ldeck", "French A1.ldeck"]);
}
