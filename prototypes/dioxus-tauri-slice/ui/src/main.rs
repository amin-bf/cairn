...
//!
//! Option C: **Dioxus as the Tauri frontend.** Dioxus's component model and hot reload, Tauri's
//! native shell and shipping story.
//!
//! Compiled to `wasm32-unknown-unknown` by `dx` and served by Tauri on desktop/Android, or as a
//! plain static SPA on web. Note what that costs: unlike the standalone Dioxus slice, this crate is
//! **always wasm**, so the storage backend is a runtime branch, not a `#[cfg]` — see `store.rs`,
//! which is byte-identical to the Leptos slice's.
...
fn App() -> Element {
    ...
    use_future(move || async move {
        status.set(store::backend().await);
    ...
    let grade = move |g: u8| {
        spawn(async move {
            ...
            if let Err(e) = store::append(&ev).await {
                status.set(format!("append failed: {e}"));
            }
            match store::read_all().await {
                Ok(evs) => log.set(evs),
                Err(e) => status.set(format!("read failed: {e}")),
    ...
    rsx! {
        ...
        main {
            ...
            section { class: "log",
                ...
                p { class: "hint", "HOT RELOAD PROOF — edited while the Tauri window was open." }
            }
        }
    }
}
...
.back { font-size: 1.2rem; text-align: center; color: #7ee2ba; border-top: 1px solid #2a2f39;
...
.next { width: 100%; margin-top: 1rem; padding: 0.9rem; font: inherit; font-weight: 600;
        color: #14161a; background: #7ee2ba; border: 0; border-radius: 10px; cursor: pointer; }
.log { margin-top: 2rem; }
