...
impl State {
    ...
    fn handle_platform_output_inner(
        ...
        if let Some(ime) = ime {
            // PROTOTYPE PATCH for leitner #67 — the only change in this vendored copy.
            //
            // On Android this block re-pops the soft keyboard on **every tap into a text field**.
            // `Memory::request_focus` interrupts IME composition unconditionally, and `TextEdit`
            // calls it on every pointer interaction without checking whether the widget is already
            // focused — so `should_interrupt_composition` is set by an ordinary tap, including a tap
            // on the field that already has focus.
            //
            // The interruption is implemented as hide-then-show, and winit's Android backend maps
            // `set_ime_allowed` onto `hide_soft_input`/`show_soft_input`. So the cost is a visible
            // keyboard dismiss-and-reopen per tap, which also collapses and restores the IME inset
            // and therefore resets the scroll position.
            //
            // It buys nothing on this platform: winit's Android backend handles only motion and key
            // events and has no IME path at all, so there is never a composition to interrupt. That
            // is the same gap `AGENTS.md` client-stack rule 8 records for non-Latin text.
