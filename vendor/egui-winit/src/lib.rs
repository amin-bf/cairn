...
impl State {
    ...
    fn handle_platform_output_inner(
        ...
        if let Some(ime) = ime {
            // THE ONE CHANGE IN THIS VENDORED COPY — leitner, ADR-0026. See `vendor/PATCH.md`.
            //
            // On Android this block dismisses and reopens the soft keyboard on every tap into a
            // text field, including a tap on the field that already has focus: `TextEdit` calls
            // `request_focus` on every pointer interaction with no `has_focus` check,
            // `request_focus` interrupts composition unconditionally, and winit's Android backend
            // maps the hide-then-show below onto `hide_soft_input`/`show_soft_input`. It buys
            // nothing there — that backend has no IME path at all, so there is never a composition
            // to interrupt. Measured: 6 hides / 17 shows for three taps, against 0 and 0 guarded.
            //
            // The guard is bound to this block's **shape**, not its line number. A release that
            // restructures it must be re-judged, not re-applied.
